...
def main():
    ...
    while True:
        print("Waiting servoo control board connection ...")
        try:
            ret = interface.auto_open()
            if ret is True:
                print("Control board connected.")
                servo_ids = interface.search_servo_ids()
                break
        except Exception as e:
            print(str(e))
    ...
    print(f"Detected servo IDs: {servo_ids}")

    if 3 in servo_ids:
        print(f"Fore.GREEN + Servo ID {servo_ids} is responding (communication OK)" + Fore.RESET)
    else:
        print(Fore.RED + f"Servo ID {servo_ids} is NOT responding (communication FAILED)" + Fore.RESET)
